...
class A_dat extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('A_dat_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'a_dat/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'a_dat/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'a_dat/index.html';
            $config['first_url'] = base_url() . 'a_dat/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->A_dat_model->total_rows($q);
        $a_dat = $this->A_dat_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $this->load->view('a_dat/kedatangan_list', $data);
    }

    public function read($id) 
    {
        $row = $this->A_dat_model->get_by_id($id);
        if ($row) {
            $data = array(
		'no' => $row->no,
		...
		'program_yang_diambil' => $row->program_yang_diambil,
            ...
            $this->load->view('a_dat/kedatangan_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('a_dat'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('a_dat/create_action'),
	    'no' => set_value('no'),
        ...
        $this->load->view('a_dat/kedatangan_form', $data);
    }
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('a_dat/update_action'),
    ...
    public function update_action() 
    {
		...
		'program_yang_diambil' => $this->input->post('program_yang_diambil',TRUE),
            ...
            $this->A_dat_model->update($this->input->post('no', TRUE), $data);
	...
	foreach ($this->A_dat_model->get_all() as $data) {
            $kolombody = 0;
...
/* End of file A_dat.php */
/* Location: ./application/controllers/A_dat.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2023-03-28 07:11:57 */
/* http://harviacode.com */